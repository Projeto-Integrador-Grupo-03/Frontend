  ...
  function CardCategorias({categoria}: CardCategoriaProps) {
    return (
      <div className='flex flex-col rounded-2xl overflow-hidden border border-gray-300 shadow-lg h-full'>
        <header className='py-4 px-6 bg-slate-700 text-white font-bold text-2xl rounded-tl-2xl rounded-tr-2xl'>Categoria</header>
        <div className='flex flex-col justify-between flex-grow p-6 bg-white'>
          ...
          <div className="flex justify-between mt-4">
            <Link to={`/editarCategoria/${categoria.id}`} className='w-1/2 text-white bg-slate-700 hover:bg-slate-900 flex items-center justify-center py-2 rounded-bl-2xl rounded-tr-2xl'>
